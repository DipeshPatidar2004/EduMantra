...
const features = [
  ...
  { icon: "🌐", title: "Multilingual Support", desc: "Learn in your native language." },
  { icon: "👥", title: "Community Learning", desc: "Connect with peers & mentors." },
  { icon: "📊", title: "Smart Analytics", desc: "Track progress & insights." },
  { icon: "🎮", title: "Gamified Experience", desc: "Earn badges & rewards." },
];
...
router.get("/stats", (_, res) => res.json(stats));
router.get("/features", (_, res) => res.json(features));
router.get("/benefits", (_, res) => res.json(benefits));
